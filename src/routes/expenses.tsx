...
function ExpensesPage() {
  ...
  const catName = (id: keyof ExpenseBreakdown) => t(`cat.${id}`);
  const donut = cats.map((c) => ({ name: catName(c.id), value: c.actual, id: c.id }));

  const trend = MONTHLY.map((mm) => {
    const row: Record<string, number | string> = { label: mShort(mm.label) };
    (Object.keys(CATEGORY_LABEL) as Array<keyof ExpenseBreakdown>).forEach((k) => {
      row[catName(k)] = mm.expenseBreakdown[k];
    });
    return row;
  });
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <KPICard label={t("kpi.totalExpenses")} value={formatConverted(m.expenses, "USD", home, { compact: true })} subtitle={`${mShort(m.label)} ${m.year}`} />
        <KPICard label={t("kpi.vsBudget")} value={formatConverted(variance, "USD", home, { compact: true })} delta={variancePct} tone={variance > 0 ? "danger" : "success"} subtitle={`${t("kpi.budget")} ${formatConverted(totalBudget, "USD", home, { compact: true })}`} />
        <KPICard label={t("kpi.largestCategory")} value={catName(largest.id)} subtitle={formatConverted(largest.actual, "USD", home, { compact: true })} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <h3 className="text-sm font-semibold mb-2">{t("card.expenseBreakdown")}</h3>
          <p className="text-xs text-muted-foreground mb-2">{t("card.expenseBreakdownSub")}</p>
          <div className="h-56">
          ...
          <div className="flex flex-wrap gap-2 mt-3 text-[11px]">
            ...
            {filter && <button onClick={() => setFilter(null)} className="text-xs text-muted-foreground underline">{t("common.clearFilter")}</button>}
          </div>
        </Card>

        <Card>
          <h3 className="text-sm font-semibold mb-2">{t("card.monthlyTrend")}</h3>
          <p className="text-xs text-muted-foreground mb-2">{t("card.monthlyTrendSub")}</p>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trend} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                ...
                {(Object.keys(CATEGORY_LABEL) as Array<keyof ExpenseBreakdown>).map((k, i) => (
                  !hidden.has(k) && (
                    <Line key={k} type="monotone" dataKey={catName(k)} stroke={COLORS[i % COLORS.length]} strokeWidth={1.75} dot={false} />
                  )
                ))}
              </LineChart>
            </ResponsiveContainer>
      ...
      <Card>
        <h3 className="text-sm font-semibold mb-3">{t("card.budgetVsActual")}</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="text-left text-muted-foreground border-b border-border">
                <th className="py-2 pr-2 font-medium">{t("common.category")}</th>
                <th className="py-2 pr-2 font-medium text-right">{t("common.budget")}</th>
                <th className="py-2 pr-2 font-medium text-right">{t("common.actual")}</th>
                <th className="py-2 pr-2 font-medium text-right">{t("common.variance")}</th>
                <th className="py-2 pr-2 font-medium text-right">{t("common.variancePct")}</th>
              ...
              {displayedCats.map((c, idx) => {
                ...
                return (
                  <tr key={c.id} className={cn("border-b border-border/60", idx % 2 === 1 ? "bg-muted/20" : "")}>
                    <td className="py-2 pr-2 font-medium">{catName(c.id)}</td>
                    ...
                    <td className="py-2 pr-2">
                      {over && <span className="inline-flex items-center gap-1 text-destructive"><AlertTriangle className="h-3 w-3" />{t("status.over")}</span>}
                      {slightlyOver && <span className="text-warning-foreground">{t("status.near")}</span>}
                      {!over && !slightlyOver && <span className="text-success">{t("status.onTrack")}</span>}
                    </td>
                    <td className="py-2 pr-2">
      ...
      <Card>
        <h3 className="text-sm font-semibold mb-3">{t("common.month")}</h3>
